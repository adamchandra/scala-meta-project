...
class MyJavaWriter(classfile: Classfile, writer: Writer) extends CodeWriter(writer) {
  ...
  def nameToClass(str: String): String = {
    ...
    if (res == "java.lang.Object") "Any" else shortenFQN(res)
  }

  def nameToClass0(str: String) = {
    val res = Names.decode(str.replace('/', '.'))
    if (res == "java.lang.Object") "AnyRef" else shortenFQN(res)
  }
  ...
  def printMethod(flags: Int, name: Int, tpe: Int, attribs: List[cf.Attribute]) {
      ...
      mp.parse match {
        case None =>
          if (getName(name) == "<init>") {
            ...
            print("def " + color(Console.GREEN, Names.decode(getName(name))))
            print(getType(tpe) + ";").newline
          }
        case Some(str) =>
          if (getName(name) == "<init>")
            ...
            print("def " + color(Console.GREEN, Names.decode(getName(name))) + str + ";").newline
      }
      case None =>
        if (getName(name) == "<init>") {
          print("def this" + color(Console.GREEN, getType(tpe)) + ";").newline
        } else {
          print("def " + color(Console.GREEN, Names.decode(getName(name))))
          print(getType(tpe) + ";").newline
        }
    ...
    attribs find {
      ...
      for (i <- Iterator.range(0, n) map {x => 2 * (x + 1)}) {
        ...
        if (i > 2) print(", ")
        print(color(Console.YELLOW, getClassName(inx).trim()))
      }
      undent.newline
      case None =>
    }
  ...
  def printClassHeader {
    ...
    cf.interfaces foreach {
      n => print(" with " + color(Console.YELLOW, getClassName(n)))
    }
  }
