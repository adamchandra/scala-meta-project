...
// }
// 
// private[util] sealed abstract class Iterable[T] extends NotNull
// {
// 	def foreach(f: T => Unit) = toList.foreach(f)
// 	def toList: List[T]
// }
// private[util] sealed trait Removable[T] extends NotNull
// {
// 	def -=(t: T) : Unit
// 	def --=(all: Iterable[T]) { all.foreach(-=) }
// 	def --=(all: scala.Iterable[T]) { all.foreach(-=) }
// }
// private[util] sealed trait Addable[T] extends NotNull
// {
// 	def +=(t: T) : Unit
// 	def ++=(all: Iterable[T]) { all.foreach(+=) }
// 	def ++=(all: scala.Iterable[T]) { all.foreach(+=) }
// }
// private[util] sealed abstract class Set[T] extends Iterable[T]
// {
// 	def contains(t: T): Boolean
// }
// private[util] sealed class SetWrapper[T](val underlying: JSet[T]) extends Set[T]
// {
// 	def contains(t: T) = underlying.contains(t)
// 	def toList =Wrappers.toList(underlying.iterator)
// }
// private[util] final class MutableSetWrapper[T](wrapped: JSet[T]) extends SetWrapper[T](wrapped) with Addable[T] with Removable[T]
// {
// 	def +=(t: T) { underlying.add(t) }
// 	def -=(t: T) { underlying.remove(t) }
// 	def readOnly: Set[T] = this
// }
// private[util] sealed abstract class Map[K,V] extends Iterable[(K,V)]
// {
// 	def apply(key: K): V
...
// 		}
// }
// private[util] sealed abstract class MapWrapper[K,V](val underlying: JMap[K,V]) extends Map[K,V]
// {
...
// }
// private[util] sealed class MutableMapWrapper[K,V](wrapped: JMap[K,V]) extends MapWrapper[K,V](wrapped) with Removable[K] with Addable[(K,V)]
// {
