...
* @see sbt.PathFinder*/
abstract class Path extends PathFinder 
{
	...
	* The <code>relativePath</code> method is used to return the relative path to the base directory. */
	override def baseDir : Path = new BaseDirectory(this)

	private[util] def addTo(pathSet: Set[Path])
	{
		if(asFile.exists)
			pathSet += this
	}
	...
	* default base directory if one is not specified explicitly using the <code>baseDir</code> operator.*/
	...
	private[util] def prependTo(s: String): String
...
private class BaseDirectory(private[util] val path: Path) extends Path
{
	override def baseDir : Path = this
	...
	private[util] def prependTo(s: String) = "." + sep + s
}
private[util] final class FilePath(file: File) extends Path
{
	...
	private[util] def prependTo(s: String) = absolutePath + sep + s
}
// toRoot is the path between this and the root project path and is used for toString
private[util] final class ProjectDirectory(file: File, toRoot: Option[Path]) extends Path
{
	...
	private[util] def prependTo(s: String) = foldToRoot(_.prependTo(s), "." + sep + s)
	private[util] def foldToRoot[T](f: Path => T, orElse: T) = toRoot.map(f).getOrElse(orElse)
}
private[util] final class RelativePath(val parentPath: Path, val component: String) extends Path
{
	checkComponent(component)
	override def toString = parentPath prependTo component
	lazy val asFile = new File(parentPath.asFile, component)
	private[util] def prependTo(s: String) =  parentPath prependTo (component + sep + s)
...
object Path
{
	...
	* reevaluated on each call to the <code>PathFinder</code>'s <code>get</code> method.  */
	def lazyPathFinder(paths: => Iterable[Path]): PathFinder = new PathFinder {
		private[util] def addTo(pathSet: Set[Path]) = pathSet ++= paths
	}
	...
	* not a slash, and not '.' or '..'.*/
	...
	private[util] def relativize(baseFile: File, file: File): Option[String] =
	{
		val pathString = file.getAbsolutePath
	...
	private[util] def basePathString(basePath: Path): Option[String] = baseFileString(basePath.asFile)
...
* has changed.*/
abstract class PathFinder extends NotNull
{
	...
	* @see Path.baseDir
	*/
	def baseDir : PathFinder = new BasePathFinder(this)
	...
	* time of calling.  If the filesystem changes, two calls to this method might be different.*/
	final def get: scala.collection.mutable.Set[Path] =	{
		var pathSet = new HashSet[Path]
		addTo(pathSet)
    pathSet
		// Wrappers.readOnly(pathSet)
	}
...
private abstract class FilterPath extends PathFinder with FileFilter
{
	...
	{
		for(matchedFile <- Util.wrapNull(path.asFile.listFiles(this)))
...
private class DescendentOrSelfPathFinder(val parent: PathFinder, val filter: FileFilter) extends FilterPath
{
	...
	private def handlePathDescendent(path: Path, pathSet: Set[Path])
	{
		...
		for(childDirectory <- Util.wrapNull(path.asFile.listFiles(DirectoryFilter)))
