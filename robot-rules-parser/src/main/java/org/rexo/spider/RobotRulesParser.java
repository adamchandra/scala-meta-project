...
public class RobotRulesParser {
  private static Logger log = Logger.getLogger( RobotRulesParser.class );
	...
	public static class RobotRuleSet {
		...
		public boolean isAllowed(String path)
		{
			...
			{
				// just ignore it - we can still try to match path prefixes
			}
    ...
    public String toLogString() {
      ...
      return buf.toString();
		...
		public String toString()
		{
			...
			for(int i = 0; i < entries.length; i++) 
				if(entries[i].allowed)
					buf.append("Allow: " + entries[i].prefix + System.getProperty("line.separator"));
				else 
					buf.append("Disallow: " + entries[i].prefix + System.getProperty("line.separator"));
			return buf.toString();
		}
	}
	...
	public RobotRuleSet parseRules(byte robotContent[])
	{
		...
		while(lineParser.hasMoreTokens())
		{
			...
			else if(line.length() >= 9 && line.substring(0, 9).equalsIgnoreCase("Disallow:"))
			{
				...
				{
					log.error("Error parsing robots rules - can't decode path: " + path);
				}
	...
	public static void main(String argv[])
	{
		...
		{
			log.info("Usage:");
			log.info("   java <robots-file> <url-file> <agent-name>+");
			log.info("");
			log.info("The <robots-file> will be parsed as a robots.txt file,");
			log.info("using the given <agent-name> to select rules.  URLs ");
			log.info("will be read (one per line) from <url-file>, and tested");
			log.info("against the rules.");
			System.exit(1);
		}
		...
		try {
			...
			log.info("Rules:");
			log.info(rules.toLogString() + "\n");

			String testPath = testsIn.readLine().trim();
			while(testPath != null)
			{
				log.info((rules.isAllowed(testPath) ? "allowed" : "not allowed") + ":\t" + testPath);
