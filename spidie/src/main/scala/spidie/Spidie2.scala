...
case class Page(url:URL, date:Date, content:String) extends MongoObject {
  ...
  def links: Seq[URL] = dom match {
    case Some(dom) => (dom \\ "a").flatMap(node => (node \ "@href").headOption match {
      case Some(nodeseq) => {
        try { val u = Address.normalizedURL(url, nodeseq.head.toString); /* println(u); */ Some(u) }
        catch { case e:Exception => { System.err.println("Skipping href "+nodeseq.head); Nil } }
      }
...
object Address extends MongoObjectShape[Address] {
  ...
  override def factory(dbo: DBObject): Option[Address] =
    for {host(h) <- Some(dbo); url(u) <- Some(dbo); date(d) <- Some(dbo); status(st) <- Some(dbo)} yield
