...
object LoadURL extends SpidieLogging {
  ...
  def asString(url:URL): (URL, Int, String, String) = {
    ...
    val contentType = try { connection.getContentType.toLowerCase.split("[; ]+")(0) } catch { case e:Exception => { error(e); "ContentTypeError"} }
    ...
    contentType match {
      case "text/html" => {
        ...
        try {
          ...
          case e:Exception => { 
            error("Failed to load "+url+": "+e.toString)
