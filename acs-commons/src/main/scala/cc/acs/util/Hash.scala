...
object Hash {
	...
	def apply(algorithm: String, s: String): Array[Byte] = apply(algorithm, new ByteArrayInputStream(s.getBytes("UTF-8")))
	/** Calculates the hash of the given file.*/
	def apply(algorithm: String, file: File): Array[Byte] = apply(algorithm, new java.io.FileInputStream(file))
	/** Calculates the hash of the given stream, closing it when finished.*/
	def apply(algorithm: String, stream: InputStream): Array[Byte] = {
